...
RUST=rustc
RFLAGS=-C opt-level=1 -Z no-landing-pads --target $(MACHINE)-none-eabi -g --emit obj -L libcore-$(MACHINE)
...
# Pure Rust kernel
kernel.o : kernel.rs libcore-$(MACHINE)/libcore.rlib
	$(RUST) $(RFLAGS) -o $@ $<

# Target hardware library
librpi-$(MACHINE)/librpi.rlib : rpi/src/lib.rs \
			$(wildcard rpi/src/*.rs)
	$(RUST) $(RLFLAGS) -L libcore-$(MACHINE) --out-dir librpi-$(MACHINE) $<
